...
let allCards = document.querySelector(".all-card");
let favoriteArr = JSON.parse(localStorage.getItem("favBooks")) || [];
books.forEach((element) => {
  allCards.innerHTML += `
     <div class="col col-3 my-3">
    <div class="card" style="width: 18rem">
      <div class="card-body">
        <h5 class="card-title">${element.authors}</h5>
        <p class="card-text">
         ${element.publisher}
        </p>
        <p class="card-text">
        ...
        <button class="btn btn-primary " id="${element.id}">Add to Favorite</button>
      </div>
    </div>
  </div> 
    `;
...
allButtons.forEach((item) => {
  item.addEventListener("click", function () {
    let click = books.find((el) => el.id === +item.id);

    favoriteArr.push(click);
    // console.log(item.id);

    localStorage.setItem("favBooks", JSON.stringify(favoriteArr));
  });
});
